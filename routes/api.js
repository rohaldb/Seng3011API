...
const access_token = 'EAACEdEose0cBACKKlyrPqkMAmqJhZB1TUnzSpyWpscHP9ZBGoc8byo8Rc9VbgZBGbK1KuTgnoQZBCvVkC0JwYj9hZC619SkxXLY8Pza2tOqLLaMQVh9iTuZBQcbJaxNcl0vuXb9msaNUKhZAig0FN7wVdxKhXeUGZAqvxkKmHxYMQyG9332upIdStZBGdqCBmp4MZD'
...
router.get('/', function (req, res, next) {
    ...
    if (!start_date.isValid() || !end_date.isValid() || !company) {
        ...
        fetch(`https://graph.facebook.com/${company}?since=${start_date.unix()}&until=${end_date.unix()}&fields=${statistics}&access_token=${access_token}`).then(function (response) {
            if (response.ok) {
            ...
            if (response != undefined) {
                response.json().then(data => {
                    if (data.data[0]) {
                        ...
                        return fetch(`https://graph.facebook.com/${companyId}?since=${start_date.unix()}&until=${end_date.unix()}&fields=${statistics}&access_token=${access_token}`)
                    } else {
