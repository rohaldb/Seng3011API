 ...
 * The start_time and end_time will reflect the original request if cached.
 */
var REDIS_URL = process.env.REDIS_URL || 'redis://localhost:6379'
var cache = require('express-redis-cache')({
  client: require('redis').createClient(REDIS_URL),
    expire: {
      200: 300, /* expire successful requests every 5 minutes */
...
router.get('/', cache.route(), function (req, res, next) {
  ...
  res.status(400)
  res.json(failureResponseFormatter(start_time, req, 400, 'No endpoint specified'))
})
...
const getAccessToken = (req, res, callback) => {
  ...
  fs.stat('.token', function(err, stats) {
...
router.get('/post/:id', cache.route(), function (req, res, next) {
  ...
  getAccessToken(req, res, function(token) {
    if (token === 'error') {
      res.status(400)
      res.json(failureResponseFormatter(start_time, req, 400, 'Missing parameter `access_token`'))
    } else {
      access_token = token
    ...
    if (validPostStats(statistics) !== '') {
      ...
      res.status(400)
      res.json(failureResponseFormatter(start_time, req, 400, validPostStats(statistics)))
    }
    ...
    .then(function (response) {
      if (response.ok) {
        response.json().then(data => {
          if (data.error) {
            res.status(400)
            res.json(failureResponseFormatter(start_time, req, 400, data.error.message))
          } else {
            res.json(successResponseFormatter(start_time, req, response, formatPostInfo(data)))
          }
        })
      } else {
        response.json().then(data => {
...
router.get('/:company', cache.route(), function (req, res, next) {
  ...
  getAccessToken(req, res, function(token) {
    ...
    if (validCompanyStats(statistics) != '') {
      /* check for valid stats parameter */
      res.status(400)
      res.json(failureResponseFormatter(start_time, req, 400, validCompanyStats(statistics)))
    }
    statistics = preprocessQuery(statistics)
    ...
    db.each(`SELECT *, COUNT(1) > 0
      ...
      if (!start_date.isValid() || !end_date.isValid()) {
        ...
        res.status(400)
        res.json(failureResponseFormatter(start_time, req, 400, 'Invalid date parameters'))
      } else if (has_start && has_end && !end_date.isAfter(start_date)) {
        /* check that start_date <= end_date */
        res.status(400)
        res.json(failureResponseFormatter(start_time, req, 400, 'start_date must precede end_date'))
      } else {
        ...
        .then(response => {
          if (response.ok) {
              ...
              if (!statistics.match(/posts/)) {
                ...
                .then(response => {
                  if (response.ok) {
                    ...
                    res.status(400)
                    res.json(failureResponseFormatter(start_time, req, 400, 'An unknown error occured'))
                  }
                })
              }
            }).catch(error => console.error(error))
            ...
            response.json().then(data => {
              if (data && data.error && data.error.message && data.error.message.match(/access token|temporarily disabled/i)) {
                /* inform users of invalid or expired access token */
                res.status(400)
                res.json(failureResponseFormatter(start_time, req, 400, 'Access token invalid or expired'))
              } else {
                /* somewhat informative error */
                res.status(400)
                res.json(failureResponseFormatter(start_time, req, 400, `Unknown company \`${req.params.company}\``))
              }
            }).catch(error => console.error(error))
          }
        }).catch(error => console.error(error))
 ...
 */
const successResponseFormatter = (start_time, req, api_response, api_data) => {
  const metadata = metaDataGen(start_time, req, api_response.status, api_response.statusText, api_data)
  return {
    data: api_data,
    ...metadata
  }
 ...
 */
const failureResponseFormatter = (start_time, req, response_code, status_text) => {
  const metadata = metaDataGen(start_time, req, response_code, status_text)
  return {
    data: [],
    ...metadata
  }
 ...
 */
const metaDataGen = (start_time, req, response_code, status_text, api_data = null) => {
